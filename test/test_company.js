...
contract("Company", (accounts) => {
    ...
    it("Should not add a company if not owner", async () => {
        try {
            await company.addCompany(companyAddress, "Test Company", { from: companyAddress });
        } catch (e) {
            assert(e.message.includes("Only contract owner can execute this function"));
            return;
        }
        assert(false);
    });
    ...
    it("Should add a project", async () => {
        await company.addProject("Test Project", "Test Description", 1000);
