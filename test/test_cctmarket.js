...
contract("CarbonCreditMarket", function (accounts) {
  ...
  before(async () => {
    ...
    await validatorRegistryInstance.addValidator(validatorAddress, {
      from: owner,
    });
  ...
  it("Should add a company", async () => {
    ...
    assert.strictEqual(
      companyInstanceData,
      "Test Company",
      "Company should be added with the correct name"
    );
  });
  ...
  it("Should not add a company if not owner", async () => {
    try {
      ...
      assert.include(
        ...
        "Error message should contain correct restriction message"
      );
    }
  });
  ...
  it("Should add a project", async () => {
    ...
    assert.strictEqual(
      ...
      "Test Project",
      "Project name should be correctly recorded"
    );
    assert.strictEqual(
      projectData.cctAmount.toNumber(),
      3,
      "CCT amount should be set correctly"
    );
  });
  ...
  it("Selling CCT from Project", async () => {
    ...
    const projectData = await companyInstance.projects(0);
    assert.strictEqual(
      projectData.cctListed.toNumber(),
      3,
  ...
  it("Buying CCT from a project should update cctSold and emit BuyCredit event", async () => {
    ...
    let buy1 = await carbonCreditMarketInstance.buy(
      ...
      companyAddress,
      projectId,
      { from: buyerAddress, value: oneEth * cctToBuy }
    );
  ...
  it("Should handle valid project validation by a registered validator (no penalties)", async () => {
    ...
    const buyersAddresses = await carbonCreditMarketInstance.getProjectBuyers(
      projectId
    );
    ...
    let validate = await carbonCreditMarketInstance.validateProject(
      companyAddress,
      projectId,
      true,
      actualCCT,
    ...
    assert.strictEqual(
      projectData.state.toString(),
      "1",
      "Project state should be marked as completed"
    );
    ...
    truffleAssert.eventEmitted(
      ...
      (ev) => {
        return (
          ev.isValid === true &&
          ev.projectId.toNumber() === projectId &&
          ev.companyAddress === companyAddress
        );
      },
    ...
    for (let { buyer, initialCCT } of buyersInitialCCT) {
      ...
      assert(
        newCCT.gt(initialCCT),
        `CCT for buyer ${buyer} should have increased`
      );
    }
  });
  ...
  it("Should revert when attempting to validate a project that is already completed", async () => {
    ...
    await carbonCreditMarketInstance.validateProject(
      companyAddress,
      projectId,
      true,
      actualCCT,
    ...
    try {
      ...
      assert.include(
        ...
        "Error message should indicate that the project cannot be re-validated"
      );
    }
  });
  ...
  it("Should correctly handle excess CCT upon successful project validation", async () => {
    ...
    assert(
      newProjectIndex.sub(projectIndex).eq(web3.utils.toBN(1)),
      "One new project should be added"
    );

    const projectId = newProjectIndex.toNumber() - 1; // Assuming projects are 0-indexed

    // List and simulate selling some CCT
    await carbonCreditMarketInstance.sell(listedCCT, projectId, {
      from: companyAddress,
      value: web3.utils.toWei("13", "ether"),
    });
    await carbonCreditMarketInstance.buy(soldCCT, companyAddress, projectId, {
      from: buyerAddress,
    ...
    assert.strictEqual(
      remainingCCT.toNumber(),
      expectedRemainingCCT,
      `Remaining CCT should be set to actual CCT minus sold CCT, expected ${expectedRemainingCCT}`
    );
    ...
    truffleAssert.eventEmitted(
      ...
      (ev) => {
        return (
          ev.isValid === true &&
          ev.projectId.toNumber() === projectId &&
          ev.companyAddress.toLowerCase() === companyAddress.toLowerCase()
        );
      },
      "Project should be validated as true"
    );
